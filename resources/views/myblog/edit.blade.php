...
<div class="container-fluid">
    <div class="card">
        ...
        <form action="{{ route('myblog.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST') <!-- This line is not strictly necessary but can be used for clarity -->
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4 form-group">
                        <label for="title" class="req_fld">Title</label>
                        <input class="form-control" type="text" name="title" value="{{ old('title', $blog->title) }}">
                        @error('title')
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        @enderror
                        @if($blog->img)
                            <img src="{{ asset('storage/' . $blog->img) }}" alt="Blog Image" class="mt-2" style="width: 100px;">
                        @endif
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12 form-group">
                        ...
                        <textarea class="form-control" name="content" id="content" cols="90" rows="10" autocomplete="off">{{ old('content', $blog->content) }}</textarea>
                        @error('content')
                            <span class="text-danger small">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-4 form-group">
                        ...
                        <select class="form-control" name="status" id="status">
                            <option value="1" {{ old('status', $blog->status) == '1' ? 'selected' : '' }}>Publish</option>
                            <option value="2" {{ old('status', $blog->status) == '2' ? 'selected' : '' }}>Save as Draft</option>
                        </select>
                    </div>
                    <div class="col-md-12 form-group">
